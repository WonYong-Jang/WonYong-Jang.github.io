...
subtitle: "Driver, Executor, Node, Task, Cluster Manager/ RDD / Hadoop"    
comments: true
categories : Spark
date: 2021-04-11
...
- Spark Driver가 실행되면 main()이 실행되며 Spark Context를 만든다.   
- Spark Driver 내의 Spark Context는 Cluster Manager와 통신하여(Yarn, Kubernetes 등) Executor를 할당 받는다.   
- Spark Driver는 Executor에게 사용자가 작성한 코드 실행을 위한 Spark Context 내의 Jar, Python File등을 
전송한다.   
- Spark Driver 내의 Spark Context는 사용자의 코드를 바탕으로 실행 계획을 만들어 최종적으로는 Task를 Executor에게 전달한다.   
...
### 4-2) Parallel Processing of RDD    
