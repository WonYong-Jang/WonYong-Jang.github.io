...
여기서 튜닝 사례 등과 같은 정보는 Vector DB에 직접 넣고 관리하면서 수정, 삭제하게 되면 히스토리 추적이 어렵다.  
`따라서 원본 문서를 별도로 관리하고 이를 Vector DB에 동기화하는 구조가 운영하기 훨씬 수월하다.`     
이를 위해서 사내 문서 또는 github의 md 파일로 관리하고 이를 주기적으로 동기화 하는 스트립트 작성이 필요하다.   
