...
주의해야할 점은 모든 skew data를 AQE가 해결해주지 못한다.  

첫 번째는 skewedPartitionThresholdInBytes 와 skewedPartitionFactor 의 옵션에 충족하지 못한 경우이다.
이러한 경우는 옵션을 조절하여 해결이 가능하다.

두 번째는 AQE 가 쪼갤수 없는 형태인 경우이다.    



세 번째는 AQE 는 join 과정에서 발생하는 skew 만 
