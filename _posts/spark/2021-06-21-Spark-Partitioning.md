...
대부분의 분산 처리 시스템은 데이터를 자동으로 파티션한다. 예를 들어, 
    HDFS는 파일을 블록으로 나누고, 이 블록들을 파티션과 1:1로 매칭하여 
    다양한 노드에 분산 저장한다.   

하지만, 사용자가 직접 파티셔닝 방식을 정의하고 조절할 수 있다.   

이렇게 파티션의 크기와 수를 조정하고 파티션을 배치하는 방법을 
설정하여 RDD의 구조를 제어하는 것을 파티셔닝 이라고 한다.   

파티션의 크기나 수를 어떻게 변경하느냐에 따라 어플리케이션의 성능이 
크게 변할 수 있으므로 스파크의 동작을 잘 이해하고 적절하게 
설정해야 한다.   

`통상적으로 파티션의 크기가 클수록 메모리가 더 필요하고, 파티션의 수가 
많을 수록 core가 더 필요하다.`      
