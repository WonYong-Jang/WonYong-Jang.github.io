...
그렇기 때문에 트랜잭션을 이용하여 해당 메서드가 전부 완료되거나 에러가 날경우는 처음상태로 돌려 놓는 
...
### Spring Transaction Exception 상황에서 Rollback  
...
`선언적 트랜잭션을 사용할 때 예외에 따른 롤백처리는 Checked 예외는 롤백되지 않고, Unchecked 예외는 롤백된다!`   
