...
## 4. Filter Context 와 Query Context를 함께 사용    
...
위 예제에서 bristol 문자열로 역색인이 되기 때문에 Bristol 로 검색을 시도했을 때 찾을 수 없다.    

address의 원본 데이터가 Bristol 라면, text 타입의 경우는 기본적으로 lowercase 되어 역색인 된다.   
keyword 타입의 경우는 기본적으로 동일한 값이 역색인 된다.   

따라서, 위 쿼리처럼 term 쿼리를 이용하여 text 타입의 address: Bristol로 검색을 시도하면 검색되지 않는다.  

`공식문서에서 term 쿼리는 text 타입 검색을 피하고, 정확한 값을 검색할 수 있는 keyword와 같은 타입 검색 
사용을 권장하고 있다.`   
