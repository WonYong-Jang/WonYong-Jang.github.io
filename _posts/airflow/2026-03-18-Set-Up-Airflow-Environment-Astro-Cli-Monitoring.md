...
이제 아래와 같이 각 helm 차트를 생성 하려고 한다.   

```sh
astro-3.2.2/
  ├── dags/ ...
  └── deploy/
      ├── helmfile.yaml # 릴리스 4개 + repo + 의존순서 선언
      └── values/
          ├── statsd.yaml
          ├── airflow.yaml
          ├── prometheus.yaml
...
`helmfile 을 사용하게 되면 repositories 블록을 보고 repo를 등록한 뒤 release 를 helm upgrade --install 을 해주게 된다.`   

`즉 각 차트마다 설치해줄 필요 없이 helmfile이 한번에 sync 해주며, 실행 순서도 needs 를 통해 제어해주게 된다.`   
...
releases:  
  ...
  - name: statsd-exporter  
    ...
    values: [values/statsd.yaml]  
  
  - name: vmagent  
    namespace: monitoring  
    chart: vm/victoria-metrics-agent  
    values: [values/vmagent.yaml]  
    needs: [monitoring/prometheus, monitoring/statsd-exporter]   # 순서 보장  
  
  - name: airflow  
    namespace: airflow  
    chart: apache-airflow/airflow  
    values: [values/airflow.yaml]  
    needs: [monitoring/statsd-exporter]
```
...
statsd-exporter의 경우 statsd-exporter-prometheus-statsd-exporter가 서비스 이름이고, 같은 namespace 끼리 통신은 해당 서비스 이름으로만 충분하지만, 다른 namespace 끼리 통신할 때는 monitoring.svc.cluster.local 과 같은 이름을 추가해 주어야 한다.  
...
```yaml
# Astro 이미지 대신 차트 기본 이미지로 교체  
# images:  
#   airflow:  
#     repository:  
#       tag:  
  
# 차트 내장 stats 끄기(외부 statsd-exporter 사용을 위한)  
statsd:  
  enabled: false  
  
# airflow가 외부 statsd-exporter로 메트릭 전송하도록 설정  
# statsd_host는 실제 서비스 FQDN으로 맞춰야한다.  
# cross-namespace라서 짧은 이름이 아니라 .monitoring.svc.cluster.local 까지 적어야 한다.  
config:  
  metrics:  
    statsd_on: 'True'  
...
##### deploy/values/prometheus.yaml

```yaml
server:  
  extraFlags:  
    - web.enable-remote-write-receiver   # ★ remote_write 수신 엔드포인트 활성화  
  persistentVolume:  
    enabled: false        # 로컬 학습용이면 끔  
  
# 이 스택은 vmagent가 scrape하므로 prometheus 자체 scrape/alertmanager는 최소화  
alertmanager:  
  enabled: false  
...
config:  
  scrape_configs:  
    - job_name: airflow-statsd  
      static_configs:  
        - targets:  
            - statsd-exporter-prometheus-statsd-exporter.monitoring.svc.cluster.local:9102
```
...
helmfile -f deploy/helmfile.yaml status
