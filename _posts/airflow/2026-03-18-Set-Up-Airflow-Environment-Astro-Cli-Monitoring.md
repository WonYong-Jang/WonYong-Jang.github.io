...
subtitle: Astro CLI / StatsD / Prometheus, VictoriaMetrics, vmagent 역할 / Grafana
comments: true
categories: Airflow
date: 2026-03-18
...
StatsD가 UDP로 보내는 이유는 push 방식이기 때문이며, 어플리케이션에서 초당 수천번 이상 발생하는 메트릭을 TCP와 같이 매번 연결, 도착 확인, 재전송을 하기 어렵다. 
반면, Prometheus의 경우 pull 방식이라, 일정한 주기에 맞춰서 한번에 가져오는 작업이기 때문에 TCP 방식으로 가져오게 된다.
...
> remote_write 란 수집한 메트릭을 자기 디스크에 저장하지 않고 다른 네트워크 너머의 다른 저장소로 보내는 표준방식이다.
