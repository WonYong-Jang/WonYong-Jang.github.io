...
`Task 들은 별도의 프로세스, 워커, 또는 다른 머신에서 수행되므로 Task들간 데이터 전송을 위한 메모리 공유가 불가능하다.`       
따라서, Xcom 을 사용하여 Task 들간의 데이터 전송이 가능하다.  

`주로 작은 규모의 데이터 공유를 위해 사용되며, Xcom 내용은 Default 설정으로 
